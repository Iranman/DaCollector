            ...
            foreach (var linkedEntity in entities)
            {
                switch (linkedEntity.GetTmdbEntity())
                {
                    case TMDB_Movie movie:
                        DaCollectorEventHandler.Instance.OnMovieUpdated(movie, UpdateReason.ImageRemoved);
                        break;

                    case TMDB_Show show:
                        DaCollectorEventHandler.Instance.OnSeriesUpdated(show, UpdateReason.ImageRemoved);
                        break;
                    ...
                    case TMDB_Season season:
                    {
                        ...
                        DaCollectorEventHandler.Instance.OnSeriesUpdated(tmdbShow, UpdateReason.ImageRemoved);
                        break;
                    }
                    ...
                    case TMDB_Episode episode:
                    {
                        if (episode.TmdbShow is not { } tmdbShow)
                            continue;
                        DaCollectorEventHandler.Instance.OnEpisodeUpdated(tmdbShow, episode, UpdateReason.ImageRemoved);
                        break;
                    }
                }
            }
