...
public static partial class AVDumpHelper
{
    ...
    public static AVDumpSession DumpFiles(IEnumerable<KeyValuePair<int, string>> videos, bool synchronous = false)
    {
        ...
        {
            DaCollectorEventHandler.Instance.OnAVDumpStart(session);
            ...
            try
            {
                ...
                // Report the results.
                DaCollectorEventHandler.Instance.OnAVDumpEnd(session);
            }
            catch (Exception ex)
            {
                var message =
                ...
                DaCollectorEventHandler.Instance.OnAVDumpGenericException(session, ex);
            }
            finally
            {
                lock (_startLock)
    ...
    private static void OnStdErrMessage(DataReceivedEventArgs eventArgs, AVDumpSession session, StringBuilder stdErrBuilder)
    {
        ...
        DaCollectorEventHandler.Instance.OnAVDumpMessage(session, AnidbAvdumpEventType.Error, eventArgs.Data);
    }
    ...
    private static void OnStdOutMessage(DataReceivedEventArgs eventArgs, AVDumpSession session, StringBuilder stdOutBuilder)
    {
        ...
        if (result.Success)
        {
            ...
            DaCollectorEventHandler.Instance.OnAVDumpProgress(session, currentProgress);
            return;
        }
        ...
        if (result.Success)
        {
            ...
            DaCollectorEventHandler.Instance.OnAVDumpCreqUpdate(session, succeeded, failed, pending);
            return;
        }
        ...
        if (InvalidCredentialsRegex().IsMatch(eventArgs.Data))
            DaCollectorEventHandler.Instance.OnAVDumpMessage(AnidbAvdumpEventType.InvalidCredentials);

        // Emit a timeout event if the connection to anidb timed out.
        if (TimeoutRegex().IsMatch(eventArgs.Data))
            DaCollectorEventHandler.Instance.OnAVDumpMessage(AnidbAvdumpEventType.Timeout);

        if (eventArgs.Data.Trim().StartsWith("ed2k://|file|"))
        {
            var ed2kLink = eventArgs.Data.Trim();
            session.ED2Ks.Add(ed2kLink);
            DaCollectorEventHandler.Instance.OnAVDumpMessage(session, AnidbAvdumpEventType.ED2KLink, ed2kLink);
            return;
        }

        // Append everything else to the outputs. We use \r\n for v1 compatibility.
        stdOutBuilder.Append(eventArgs.Data + "\n");
        DaCollectorEventHandler.Instance.OnAVDumpMessage(session, AnidbAvdumpEventType.Message, eventArgs.Data);
    }
    ...
    private static bool PrepareAVDump(bool force = false)
    {
        lock (_prepareLock)
        {
            ...
            DaCollectorEventHandler.Instance.OnAVDumpMessage(AnidbAvdumpEventType.InstallingAVDump);

            if (string.IsNullOrEmpty(expectedVersion))
            {
                var ex = new Exception("Unable to install AVDump3 automatically. Please manually install it before continuing.");
                DaCollectorEventHandler.Instance.OnAVDumpInstallException(ex);
                logger.Error(ex);
                return false;
            }
            ...
            DaCollectorEventHandler.Instance.OnAVDumpMessage(AnidbAvdumpEventType.InstalledAVDump);
            return true;
        }
    }
    ...
    private static void ReplaceNet6And8()
    {
        ...
        {
            DaCollectorEventHandler.Instance.OnAVDumpInstallException(e);
