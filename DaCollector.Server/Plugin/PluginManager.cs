...
using DaCollector.Abstractions.Plugin.Events;
...
public partial class PluginManager(ILogger<PluginManager> logger, ISystemService systemService, IApplicationPaths applicationPaths) : IPluginManager
{
        ...
        try
        {
            foreach (var dllPath in dllsToLoad)
            {
                ...
                try
                {
                    ...
                    {
                        logger.LogWarning("Found plugin using deprecated DaCollector.Plugin.Abstractions namespace. This plugin is incompatible and needs to be updated. ({DllName}, {Version})", name, version);
                        ...
                        return new()
                        {
                            ...
                            Description = isLegacyNamespace
                                ? "This plugin uses the deprecated DaCollector.Plugin.Abstractions namespace and is incompatible with this version of DaCollector. Please update the plugin to use DaCollector.Abstractions."
                    ...
                    {
                        logger.LogInformation("Skipping DLL because the loaded assembly references a newer version of DaCollector.Abstractions than what this server supports; {DllPath}", dllPath);
                        return new()
                        {
                            // Create an unique ID for this specific version that failed to load.
                            ID = UuidUtility.GetV5($"{name}@{version}"),
                            DllName = name,
                            Name = name,
                            Description = "The plugin failed to load because it references a newer version of DaCollector.Abstractions than what this server supports.",
                            Version = version,
                            Authors = authors,
                            RepositoryUrl = repositoryUrl,
                            HomepageUrl = homepageUrl,
    ...
    private static VersionInformation? ReadVersionInformationFromAssembly(Assembly assembly, out bool isLegacyNamespace, out Dictionary<string, string?> metadataAttributeDict)
    {
        ...
        var legacyRef = referencedAssemblies.FirstOrDefault(r => r.Name is "DaCollector.Plugin.Abstractions");
