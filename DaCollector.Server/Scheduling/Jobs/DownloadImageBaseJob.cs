    ...
    private void EmitEventForRelatedEntities(IImage image, UpdateReason reason)
    {
        switch ((Source, ImageType))
        {
            case (DataSource.AniDB, ImageEntityType.Poster) when RepoFactory.AniDB_Anime.GetByAnimeID(ImageID) is { } anime:
                DaCollectorEventHandler.Instance.OnSeriesUpdated(anime, reason);
                break;
                ...
                foreach (var linkedEntity in linkedEntities)
                {
                    switch (linkedEntity.GetTmdbEntity())
                    {
                        case TMDB_Movie movie:
                            DaCollectorEventHandler.Instance.OnMovieUpdated(movie, reason);
                            break;

                        case TMDB_Show show:
                            DaCollectorEventHandler.Instance.OnSeriesUpdated(show, reason);
                            break;
                        ...
                        case TMDB_Season season:
                        {
                            ...
                            DaCollectorEventHandler.Instance.OnSeriesUpdated(tmdbShow, reason);
                            break;
                        }
                        ...
                        case TMDB_Episode episode:
                        {
                            ...
                            DaCollectorEventHandler.Instance.OnEpisodeUpdated(tmdbShow, episode, reason);
                            break;
                        }
                    }
                }
