        ...
        foreach (var animeID in animeIDsToUpdate)
        {
            ...
            if (anime is null)
            {
                ...
                if (settings.AniDb.AutomaticallyImportSeries)
                {
                    ...
                    await _anidbService.ScheduleRefreshOfAnimeByID(animeID, AnidbRefreshMethod.Remote | AnidbRefreshMethod.DeferToRemoteIfUnsuccessful | AnidbRefreshMethod.CreateDaCollectorSeries).ConfigureAwait(false);
                    countAnime++;
                }
                else
                {
