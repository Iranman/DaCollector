...
public partial class DaCollectorJsonSchemaValidator<TConfig>(ILogger logger, ConfigurationService configurationService, ConfigurationInfo info, TConfig? config, bool saveValidation, bool loadValidation, JsonSchemaValidatorSettings? settings = null) : JsonSchemaValidatorBase(settings) where TConfig : class, IConfiguration, new()
{
    ...
    protected override ICollection<ValidationError> Validate(JToken? parentToken, JToken? token, JsonSchema schema, SchemaType schemaType, string? propertyName, string propertyPath)
    {
        ...
        if ((schema.ExtensionData?.TryGetValue(DaCollectorJsonSchemaGenerator.UiDefinition, out var prop0) ?? false) && prop0 is Dictionary<string, object?> uiDefinition)
        {
            if (_loadValidation)
            {
                if (uiDefinition.TryGetValue(DaCollectorJsonSchemaGenerator.ElementEnvironmentVariable, out var prop1) && prop1 is string envVarName && Environment.GetEnvironmentVariable(envVarName) is string envVar)
                {
                    ...
                    {
                        if (uiDefinition[DaCollectorJsonSchemaGenerator.ElementType] is "enum" && uiDefinition[DaCollectorJsonSchemaGenerator.EnumDefinitions] as List<Dictionary<string, string>> is { } enumDefinitions)
                        {
            ...
            {
                if (
                    uiDefinition.TryGetValue(DaCollectorJsonSchemaGenerator.ElementEnvironmentVariable, out var prop1) &&
                    uiDefinition[DaCollectorJsonSchemaGenerator.ElementEnvironmentVariableOverridable] is false &&
                ...
                if (_saveValidation && uiDefinition.TryGetValue(DaCollectorJsonSchemaGenerator.ElementRequiresRestart, out var prop2) && prop2 is true)
                {
