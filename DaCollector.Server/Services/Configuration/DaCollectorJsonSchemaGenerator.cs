...
using DaCollector.Abstractions.Config.Components;
...
/// <summary>
/// Responsible for generating JSON schema for DaCollector configuration objects.
/// </summary>
...
public class DaCollectorJsonSchemaGenerator(JsonSerializerSettings newtonsoftJsonSerializerSettings, System.Text.Json.JsonSerializerOptions systemTextJsonSerializerOptions) : ISchemaProcessor, ISchemaNameGenerator
{
    ...
    void ISchemaProcessor.Process(SchemaProcessorContext context)
    {
        ...
        if (
            ...
            (!contextualType.Type.FullName?.StartsWith("DaCollector.Abstractions.Config.Components.") ?? false)
        )
        {
