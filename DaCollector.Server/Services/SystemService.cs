...
using DaCollector.Server.Services.Abstraction;
using DaCollector.Server.Services.Configuration;
using DaCollector.Server.Services.Connectivity;
...
using ISettingsProvider = DaCollector.Server.Settings.ISettingsProvider;
...
public class SystemService : ISystemService
{
    ...
    public async Task<IHost?> StartAsync()
    {
        try
        {
            ...
            _logger.LogInformation("DaCollector: {Version}", Version);
            ...
            .ConfigureWebHost(webHostBuilder =>
                webHostBuilder
                    ...
                    .ConfigureLogging(logging =>
                    {
                        ...
                        logging.AddFilter("DaCollector.Server.API", LogLevel.Warning);
#endif
                    })
                    .UseNLog()
    ...
    private class Startup(SystemService systemService, ILogService logService, IConfigurationService configurationService, ISettingsProvider settingsProvider, IPluginManager pluginManager)
    {
        ...
        services.AddSingleton<MediaFileReviewService>();
        services.AddSingleton<MediaReadService>();
            ...
            services.AddHttpClient("GitHub", client =>
                {
                    ...
                    client.DefaultRequestHeaders.Add("User-Agent", $"DaCollector/{systemService.Version.Version.ToSemanticVersioningString()} (https://github.com/{settingsProvider.GetSettings().Web.ServerRepoName})");
            ...
            services.AddHttpClient("PlexTarget", client =>
                {
                    ...
                    client.DefaultRequestHeaders.Add("X-Plex-Client-Identifier", "dacollector");
                    client.DefaultRequestHeaders.Add("X-Plex-Product", "DaCollector");
...
pluginManager.RegisterPlugins(services);
    ...
    private bool InitializeDatabase(DatabaseFactory databaseFactory, RepoFactory repositoryFactory, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                StartupMessage = "The database version is bigger than the supported version by DaCollector. You should upgrade DaCollector or manually restore your database from a backup.";
                StartupFailedException = new(StartupMessage);
                return false;
            }
