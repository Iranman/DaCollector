...
using DaCollector.Server.MediaInfo.Subtitles;
...
public class VideoService : IVideoService
{
    ...
    private readonly DaCollectorManagedFolderRepository _managedFolderRepository;
    ...
    )
    {
        ...
        DaCollectorEventHandler.Instance.FileDeleted += OnFileDeleted;
    ...
    {
        if (managedFolder is not DaCollectorManagedFolder folder)
            return [];
    ...
    public IManagedFolder AddManagedFolder(string name, string path, DropFolderType dropFolderType = DropFolderType.Excluded, bool watchForNewFiles = false)
        => _managedFolderRepository.SaveFolder(new DaCollectorManagedFolder()
        {
            Name = name,
            Path = path,
    ...
    {
        var managedFolder = (DaCollectorManagedFolder)folder;
    ...
    public async Task ScanManagedFolder(IManagedFolder folder, string? relativePath = null, bool onlyNewFiles = false, bool skipMylist = false, bool? cleanUpStructure = null, bool? checkFileSize = null)
        => await ScanManagedFolder((DaCollectorManagedFolder)folder, relativePath, onlyNewFiles, skipMylist, cleanUpStructure, checkFileSize);

    private async Task ScanManagedFolder(DaCollectorManagedFolder folder, string? relativePath = null, bool onlyNewFiles = false, bool skipMylist = false, bool? cleanUpStructure = null, bool? checkFileSize = null)
    {
    ...
    private string[] GetFilesInImportFolder(DaCollectorManagedFolder folder)
    {
