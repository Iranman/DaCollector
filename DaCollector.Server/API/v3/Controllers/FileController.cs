...
using DaCollector.Server.API.v3.Models.Relocation;
using DaCollector.Server.API.v3.Models.Relocation.Input;
...
using AbstractReleaseInfo = DaCollector.Abstractions.Video.Release.ReleaseInfo;
using AbstractReleaseVideoCrossReference = DaCollector.Abstractions.Video.Release.ReleaseVideoCrossReference;
using AVDump = DaCollector.Server.API.v3.Models.DaCollector.AVDump;
...
using MediaInfoDto = DaCollector.Server.API.v3.Models.DaCollector.MediaInfo;
using Path = System.IO.Path;
using ReleaseInfo = DaCollector.Server.API.v3.Models.Release.ReleaseInfo;
...
) : BaseController(settingsProvider)
{
    ...
    /// Get File Details
    /// </summary>
    /// <param name="fileID">DaCollector VideoLocalID</param>
    /// <param name="include">Include items that are not included by default</param>
    /// <returns></returns>
    [HttpGet("{fileID}")]
    ...
    public async Task<ActionResult<RelocationResult>> DirectlyRelocateFileLocation([FromRoute, Range(1, int.MaxValue)] int locationID, [FromBody(EmptyBodyBehavior = EmptyBodyBehavior.Disallow)] RelocateBody body)
    {
        ...
        if (RepoFactory.DaCollectorManagedFolder.GetByID(body.ManagedFolderID) is not { } folder)
    ...
    /// </remarks>
    /// <param name="fileID">DaCollector File ID</param>
    /// <returns></returns>
    ...
    /// </summary>
    /// <param name="fileID">DaCollector ID</param>
