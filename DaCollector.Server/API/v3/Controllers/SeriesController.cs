...
using DaCollector.Server.API.v3.Models.TMDB;
using DaCollector.Server.API.v3.Models.TMDB.Input;
...
public class SeriesController : BaseController
{
    ...
    public ActionResult OverrideSeriesTitle([FromRoute, Range(1, int.MaxValue)] int seriesID, [FromBody(EmptyBodyBehavior = EmptyBodyBehavior.Disallow)] Series.Input.TitleOverrideBody body)
    {
        ...
        if (!string.Equals(series.SeriesNameOverride, body.Title))
        {
            ...
            RepoFactory.MediaSeries.Save(series, true);
    ...
    public ActionResult<List<SeriesRelation>> GetDaCollectorRelationsBySeriesID([FromRoute, Range(1, int.MaxValue)] int seriesID)
    {
        var series = RepoFactory.MediaSeries.GetByID(seriesID);
        if (series == null)
        {
    ...
    {
        var user = User;
        var query = RepoFactory.MediaSeries.GetAll()
    ...
    /// <param name="showAll">If enabled will show recommendations across all the anidb available in DaCollector, if disabled will only show for the user's collection.</param>
        ...
        DateTime? endDate = null)
    {
        ...
        return userDataQuery
            ...
            .WhereNotNull()
            .DistinctBy(episode => episode.MediaSeriesID)
            .Select(episode => episode.MediaSeries?.AniDB_Anime)
            .WhereNotNull()
    ...
    /// <param name="showAll">If true will get a list of all available anime in dacollector, regardless of if it's part of the user's collection or not.</param>
    /// <param name="includeRestricted">Include restricted (H) series.</param>
    ...
    [NonAction]
    private static Dictionary<int, (AniDB_Anime, MediaSeries?)> GetUnwatchedAnime(
        JMMUser user,
        bool showAll,
        bool includeRestricted = false,
        IEnumerable<AniDB_Anime>? watchedAnime = null)
    {
        ...
        if (showAll)
        {
            return RepoFactory.AniDB_Anime.GetAll()
                ...
                .ToDictionary<AniDB_Anime, int, (AniDB_Anime, MediaSeries?)>(anime => anime.AnimeID,
                    anime => (anime, null));
        }
    ...
    public ActionResult<Series> GetSeriesByAnidbID([FromRoute] int anidbID, [FromQuery] bool randomImages = false,
        [FromQuery, ModelBinder(typeof(CommaDelimitedModelBinder))] HashSet<DataSourceType>? includeDataFrom = null)
    {
        var series = RepoFactory.MediaSeries.GetByAnimeID(anidbID);
        if (series == null)
        {
    ...
    /// <summary>
    /// Automagically search for one or more matches for the DaCollector Series by ID,
    ...
    /// <summary>
    /// Schedule an automagically search for one or more matches for the DaCollector
    ...
    /// <summary>
    /// Get all TMDB Movies linked to the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    ...
    /// <returns>All TMDB Movies linked to the DaCollector Series.</returns>
    ...
    /// <summary>
    /// Add a new TMDB Movie cross-reference to the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    ...
    /// <summary>
    /// Refresh all TMDB Movies linked to the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    ...
    /// <summary>
    /// Download all images for all TMDB Movies linked to the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    /// <param name="body">Body containing options for refreshing or downloading metadata.</param>
    /// <returns>
    ...
    /// <summary>
    /// Get all TMDB Movie cross-references for the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    /// <returns>All TMDB Movie cross-references for the DaCollector Series.</returns>
    ...
    /// <summary>
    /// Get all TMDB Shows linked to the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    /// <param name="include">Extra details to include.</param>
    /// <param name="language">Language to fetch some details in. Omitting will fetch all languages.</param>
    ...
    /// <summary>
    /// Add a new TMDB Show cross-reference to the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    /// <param name="body">Body containing the information about the new cross-reference to be made.</param>
    /// <returns>Void.</returns>
    ...
    /// <summary>
    /// Remove one or all TMDB Show cross-reference(s) for the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    ...
    /// <summary>
    /// Refresh all TMDB Shows linked to the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    /// <param name="body">Body containing options for refreshing or downloading metadata.</param>
    /// <returns>
    ...
    /// <summary>
    /// Download all images for all TMDB Shows linked to the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    /// <param name="body">Body containing options for refreshing or downloading metadata.</param>
    /// <returns>
    ...
    /// <summary>
    /// Get all TMDB Show cross-references for the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    /// <returns>All TMDB Show cross-references for the DaCollector Series.</returns>
    ...
    /// <summary>
    /// Shows all existing episode mappings for a DaCollector Series. Optionally
    /// allows filtering it to a specific TMDB show.
    /// </summary>
    /// <param name="seriesID">The DaCollector Series ID.</param>
    ...
    )
    {
        ...
        if (tmdbShowID.HasValue && tmdbShowID.Value > 0)
        {
            ...
            if (xref == null)
                return ValidationProblem("Unable to find an existing cross-reference for the given TMDB Show ID. Please first link the TMDB Show to the DaCollector Series.", "tmdbShowID");
        }
    ...
    /// or removing links between DaCollector episodes and TMDB episodes of any TMDB
    /// shows linked to the DaCollector series.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    ...
    )
    {
        ...
        foreach (var link in body.Mapping)
        {
            ...
            if (dacollectorEpisode is null || dacollectorEpisode.MediaSeriesID != series.MediaSeriesID)
            {
    ...
    /// <summary>
    /// Preview the automagically matched DaCollector episodes with the specified TMDB
    ...
    )
    {
        ...
        if (!tmdbShowID.HasValue)
        {
            ...
            if (xref == null)
                return ValidationProblem("Unable to find an existing cross-reference for the series to use. Make sure at least one TMDB Show is linked to the DaCollector Series.", "tmdbShowID");
    ...
    /// <summary>
    /// Automagically matches DaCollector episodes with the specified TMDB show and/or
    ...
    /// <summary>
    /// Reset all existing episode mappings for the dacollector series.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    /// <returns>Void.</returns>
    [Authorize("admin")]
    ...
    /// <summary>
    /// Shows all existing episode cross-references for a DaCollector Series grouped
    ...
    /// <summary>
    /// Get all TMDB Season indirectly linked to the DaCollector Series by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Series ID.</param>
    /// <param name="include">Extra details to include.</param>
    /// <param name="language">Language to fetch some details in.</param>
    /// <returns>All TMDB Seasons indirectly linked to the DaCollector Series.</returns>
    ...
    public async Task<ActionResult> SendWatchStatesToTraktBySeriesID([FromRoute, Range(1, int.MaxValue)] int seriesID)
    {
        ...
        await scheduler.StartJob<SendSeriesWatchStatesToTraktJob>(c => c.MediaSeriesID = seriesID);
        return Ok();
    }
    ...
    public async Task<ActionResult> GetWatchStatesFromTraktBySeriesID([FromRoute, Range(1, int.MaxValue)] int seriesID)
    {
        ...
        var scheduler = await _schedulerFactory.GetScheduler();
        await scheduler.StartJob<GetSeriesWatchStatesFromTraktJob>(c => c.MediaSeriesID = seriesID);
        return Ok();
    }
    ...
    [NonAction]
    public ParallelQuery<MediaEpisode> GetEpisodesInternal(
        MediaSeries series,
        ...
        bool fuzzy)
    {
        ...
        var episodes = series.AllAnimeEpisodes
            .AsParallel()
            .Select(episode => new { DaCollector = episode, AniDB = episode?.AniDB_Episode })
            .Where(both =>
            {
                // Make sure we have an anidb entry for the episode, otherwise,
                // just hide it.
                var dacollector = both.DaCollector;
                ...
                if (includeHidden != IncludeOnlyFilter.True)
                {
                    ...
                    if (shouldHideHidden == dacollector.IsHidden)
                        return false;
                }
                ...
                if (includeMissing != IncludeOnlyFilter.True)
                {
                    ...
                    var isMissing = dacollector.VideoLocals.Count == 0 && anidb.HasAired;
                ...
                if (includeUnaired != IncludeOnlyFilter.True)
                {
                    ...
                    var isUnaired = dacollector.VideoLocals.Count == 0 && !anidb.HasAired;
                ...
                if (includeManuallyLinked != IncludeOnlyFilter.True)
                {
                    ...
                    var isManuallyLinked = dacollector.FileCrossReferences.Any(xref => xref.IsManuallyLinked);
                ...
                if (includeWatched != IncludeOnlyFilter.True)
                {
                    ...
                    var isWatched = dacollector.GetUserRecord(user.JMMUserID)?.WatchedDate != null;
            ...
            return episodes
                .Search(
                    ...
                    ep => ep.AniDB!.GetTitles()
                        ...
                        .Append(ep.DaCollector.Title)
                        .Distinct()
                        .ToList(),
                    fuzzy
                )
                .Select(a => a.Result.DaCollector);
        }
        ...
        [FromQuery] bool fuzzy = true)
    {
        ...
        var episodes = anidbSeries.AniDBEpisodes
            .AsParallel()
            .Select(episode => new { DaCollector = RepoFactory.MediaEpisode.GetByAniDBEpisodeID(episode.EpisodeID), AniDB = episode })
            .Where(both =>
            {
                // Make sure we have an anidb entry for the episode, otherwise,
                // just hide it.
                ...
                if (includeHidden != IncludeOnlyFilter.True)
                {
                    ...
                    // Or if we should only show hidden episodes and the episode is not hidden, then hide it.
                    var shouldHideHidden = includeHidden == IncludeOnlyFilter.False;
                    var isHidden = dacollector?.IsHidden ?? false;
                ...
                if (includeMissing != IncludeOnlyFilter.True)
                {
                    ...
                    // Or if we should only show missing episodes and the episode has files, the hide it.
                    var shouldHideMissing = includeMissing == IncludeOnlyFilter.False;
                    var isMissing = dacollector is not null && dacollector.VideoLocals.Count == 0 && anidb.HasAired;
                    if (shouldHideMissing == isMissing)
                        return false;
                }
                if (includeUnaired != IncludeOnlyFilter.True)
                {
                    ...
                    // Or if we should only show unaired episodes and the episode has files, the hide it.
                    var shouldHideUnaired = includeUnaired == IncludeOnlyFilter.False;
                    var isUnaired = dacollector is not null && dacollector.VideoLocals.Count == 0 && !anidb.HasAired;
                    if (shouldHideUnaired == isUnaired)
                        return false;
                }
    ...
    public ActionResult<Episode> GetNextUnwatchedEpisode([FromRoute, Range(1, int.MaxValue)] int seriesID,
        ...
        [FromQuery, ModelBinder(typeof(CommaDelimitedModelBinder))] HashSet<DataSourceType>? includeDataFrom = null)
    {
        if (RepoFactory.MediaSeries.GetByID(seriesID) is not { } series)
            return NotFound(SeriesNotFoundWithSeriesID);
        ...
        [FromRoute] Image.ImageType imageType, [FromBody] Image.Input.DefaultImageBody body)
    {
        ...
        // Update the contract data (used by DaCollector Desktop).
        RepoFactory.MediaSeries.Save(series, false);

        DaCollectorEventHandler.Instance.OnSeriesUpdated(series, isNew ? UpdateReason.ImageAdded : UpdateReason.ImageUpdated);
    ...
    public ActionResult DeleteSeriesDefaultImageForType([FromRoute, Range(1, int.MaxValue)] int seriesID, [FromRoute] Image.ImageType imageType)
    {
        ...
        DaCollectorEventHandler.Instance.OnSeriesUpdated(series, UpdateReason.ImageRemoved);
    ...
    public ActionResult<List<Series>> PathEndsWith([FromRoute] string path)
    {
        ...
        return RepoFactory.VideoLocalPlace.GetAll()
            ...
            .SelectMany(a => a.VideoLocal?.AnimeEpisodes ?? Enumerable.Empty<MediaEpisode>())
    ...
    public ActionResult<IEnumerable<int>> GetAllYears()
        => RepoFactory.MediaSeries.GetAllYears().ToList();
    ...
    public ActionResult<IEnumerable<SeasonWithYear>> GetAllSeasons()
        => RepoFactory.MediaSeries.GetAllSeasons().Select(a => new SeasonWithYear(a.Year, a.Season)).Order().ToList();
}
