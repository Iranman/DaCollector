...
public class EpisodeController : BaseController
{
        ...
        [FromQuery] string search = null, [FromQuery] bool fuzzy = true)
    {
        ...
        var episodes = RepoFactory.MediaEpisode.GetAll()
            .AsParallel()
            .Where(episode =>
            {
                // Only show episodes the user is allowed to view.
                if (!allowedSeriesDict.TryGetValue(episode.MediaSeriesID, out var isAllowed))
                    allowedSeriesDict.TryAdd(episode.MediaSeriesID, isAllowed = user.AllowedSeries(episode.MediaSeries));
                return isAllowed;
            })
            ...
            .Where(both =>
            {
                ...
                if (anidb == null || dacollector == null)
                    return false;
        ...
        if (hasSearch)
        {
            ...
            return episodes
                ...
                .ToListResult(a => new Episode(HttpContext, a.Result.DaCollector, includeDataFrom, includeFiles, includeMediaInfo, includeAbsolutePaths, includeXRefs, includeReleaseInfo), page, pageSize);
        }

        // Order the episodes since we're not using the search ordering.
        return episodes
            .OrderBy(episode => episode.DaCollector.MediaSeriesID)
            .ThenBy(episode => episode.AniDB.EpisodeType)
            .ThenBy(episode => episode.AniDB.EpisodeNumber)
            .ToListResult(a => new Episode(HttpContext, a.DaCollector, includeDataFrom, includeFiles, includeMediaInfo, includeAbsolutePaths, includeXRefs, includeReleaseInfo), page, pageSize);
    }
    ...
    public ActionResult OverrideEpisodeTitle([FromRoute, Range(1, int.MaxValue)] int episodeID, [FromBody(EmptyBodyBehavior = EmptyBodyBehavior.Disallow)] Episode.Input.EpisodeTitleOverrideBody body)
    {
        ...
        if (!string.Equals(episode.EpisodeNameOverride, body.Title))
        {
            ...
            RepoFactory.MediaEpisode.Save(episode);
    ...
    /// </summary>
    /// <param name="episodeID">DaCollector episode ID</param>
        ...
        [FromQuery, ModelBinder(typeof(CommaDelimitedModelBinder))] HashSet<DataSourceType> includeDataFrom = null)
    {
        ...
        var episode = RepoFactory.MediaEpisode.GetByAniDBEpisodeID(anidb.EpisodeID);
        if (episode == null)
    ...
    /// <summary>
    /// Get all TMDB Movies linked directly to the DaCollector Episode by ID.
    /// </summary>
    /// <param name="episodeID">DaCollector Episode ID.</param>
    /// <param name="include">Extra details to include.</param>
    /// <param name="language">Language to fetch some details in.</param>
    /// <returns>All TMDB Movies linked directly to the DaCollector Episode.</returns>
    ...
    /// <summary>
    /// Add a new TMDB Movie cross-reference to the DaCollector Episode by ID.
    /// </summary>
    /// <param name="episodeID">DaCollector Episode ID.</param>
    ...
    /// <summary>
    /// Get all TMDB Movie cross-references for the DaCollector Episode by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Episode ID.</param>
    /// <returns>All TMDB Movie cross-references for the DaCollector Episode.</returns>
    ...
    {
        var episode = RepoFactory.MediaEpisode.GetByID(seriesID);
        if (episode == null)
            return NotFound(EpisodeNotFoundWithEpisodeID);
    ...
    /// <summary>
    /// Get all TMDB Episodes linked to the DaCollector Episode by ID.
    /// </summary>
    /// <param name="episodeID">DaCollector Episode ID.</param>
    /// <param name="include">Extra details to include.</param>
    /// <param name="language">Language to fetch some details for.</param>
    /// <returns>All TMDB Episodes linked to the DaCollector Episode.</returns>
    ...
    /// <summary>
    /// Get all TMDB Episode cross-references for the DaCollector Episode by ID.
    /// </summary>
    /// <param name="seriesID">DaCollector Episode ID.</param>
    /// <returns>All TMDB Episode cross-references for the DaCollector Episode.</returns>
        ...
        [FromRoute] Image.ImageType imageType, [FromBody] Image.Input.DefaultImageBody body)
    {
        ...
        DaCollectorEventHandler.Instance.OnEpisodeUpdated(series, episode, isNew ? UpdateReason.ImageAdded : UpdateReason.ImageUpdated);
    ...
    public ActionResult DeleteEpisodeDefaultImageForType([FromRoute, Range(1, int.MaxValue)] int episodeID, [FromRoute] Image.ImageType imageType)
    {
        ...
        DaCollectorEventHandler.Instance.OnEpisodeUpdated(series, episode, UpdateReason.ImageRemoved);
