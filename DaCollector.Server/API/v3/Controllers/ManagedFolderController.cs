...
using AbstractDropFolderType = DaCollector.Abstractions.Video.Enums.DropFolderType;
...
public class ManagedFolderController(ISettingsProvider settingsProvider, ISchedulerFactory schedulerFactory, IVideoService videoService) : BaseController(settingsProvider)
{
    ...
    public ActionResult<List<ManagedFolder>> GetAllManagedFolders()
        => RepoFactory.DaCollectorManagedFolder.GetAll()
    ...
    )
    {
        ...
        if (RepoFactory.DaCollectorManagedFolder.GetAll().Any(iF => body.Path.StartsWith(iF.Path, StringComparison.OrdinalIgnoreCase) || iF.Path.StartsWith(body.Path, StringComparison.OrdinalIgnoreCase)))
        ...
        try
        {
            ...
            var newFolder = (DaCollectorManagedFolder)videoService.AddManagedFolder(body.Name, body.Path, dropFolderType, body.WatchForNewFiles);
    ...
    {
        if (RepoFactory.DaCollectorManagedFolder.GetByID(folderID) is not { } folder)
        ...
        [FromBody(EmptyBodyBehavior = EmptyBodyBehavior.Disallow)] JsonPatchDocument<ManagedFolder> patch)
    {
        ...
        RepoFactory.DaCollectorManagedFolder.SaveFolder(serverModel);
    ...
    public ActionResult EditManagedFolder([FromBody(EmptyBodyBehavior = EmptyBodyBehavior.Disallow)] ManagedFolder body)
    {
        ...
        else if (RepoFactory.DaCollectorManagedFolder.GetByID(body.ID) == null)
        ...
        if (RepoFactory.DaCollectorManagedFolder.GetAll().ExceptBy([body.ID], iF => iF.ID)
        ...
        RepoFactory.DaCollectorManagedFolder.SaveFolder(body.GetServerModel());
