...
public class PlexWebhook : BaseController
{
    ...
    private async Task Scrobble(PlexEvent data, JMMUser user)
    {
        ...
        if (episode == null)
        {
            _logger.LogInformation(
                "No episode returned, aborting scrobble. This might not have been a DaCollectorMetadata library");
            return;
        }
        ...
        if (user == null)
        {
            _logger.LogInformation("Unable to determine who \"{AccountTitle}\" is in DaCollector, make sure this is set under user settings in Desktop", data.Account.Title);
    ...
    private (MediaEpisode, MediaSeries) GetEpisode(PlexEvent.PlexMetadata metadata)
    {
        ...
        //if (!metadata.Guid.StartsWith("com.plexapp.agents.dacollector://")) return (null, null);
