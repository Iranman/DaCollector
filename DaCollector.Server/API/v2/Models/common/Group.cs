        ...
        if (!noCast)
        {
            ...
            foreach (var xref in xrefAnimeStaff)
            {
                ...
                var role = new Role
                {
                    ...
                    character_image = APIHelper.ConstructImageLinkFromTypeAndId(ctx, ImageEntityType.Character, DataSource.DaCollector, xref.CharacterID),
                    staff = staff.Name,
                    staff_image = APIHelper.ConstructImageLinkFromTypeAndId(ctx, ImageEntityType.Creator, DataSource.DaCollector, xref.CreatorID),
