...
| --- | --- |
| `DaCollector.Setup.exe` | Windows x64. The installer opens port `8111` if you select the firewall task. |
...
6. Open the Web UI at `http://127.0.0.1:8111/webui`.
...
4. Open `http://127.0.0.1:8111/webui`.
...
Then open `http://127.0.0.1:8111/webui`.
...
```powershell
netsh advfirewall firewall add rule name="DaCollector" dir=in action=allow protocol=TCP localport=8111
```

To remove the rule:

```powershell
netsh advfirewall firewall delete rule name="DaCollector" protocol=TCP localport=8111
```
