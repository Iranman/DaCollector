...
namespace DaCollector.IntegrationTests;

/// <summary>
/// Starts the full DaCollector bootstrap against an isolated temp directory,
...
public sealed class DatabaseMigrationFixture : IDisposable
{
    ...
    {
        // Isolated data directory so this run doesn't touch a real DaCollector install.
        _tempDir = Path.Combine(Path.GetTempPath(), $"dacollector-integration-{Guid.NewGuid():N}");
        Directory.CreateDirectory(_tempDir);

        // DACOLLECTOR_HOME controls Utils.ApplicationPath. Must be set before SystemService() reads it.
        // Forward slashes avoid bad JSON escape sequences when the config service parses env vars.
        Environment.SetEnvironmentVariable("DACOLLECTOR_HOME", _tempDir.Replace('\\', '/'));
        ...
        // Mutate the live settings: disable first-run and move the web port away from 38111 so this
