...
namespace DaCollector.Abstractions.Video.Relocation;
...
public class RelocationContext(RelocationContext? context = null)
{
    ...
    /// </summary>
    public IReadOnlyList<IDaCollectorEpisode> Episodes { get; init; } = context?.Episodes ?? [];

    /// <summary>
    /// Short-cut to all the series linked to the video being moved.
    /// </summary>
    public IReadOnlyList<IDaCollectorSeries> Series { get; init; } = context?.Series ?? [];

    /// <summary>
    /// Short-cut to all the groups linked to the video being moved.
    /// </summary>
    public IReadOnlyList<IDaCollectorGroup> Groups { get; init; } = context?.Groups ?? [];
}
